...
results <- results %>%
  ...
  select(term, exp_estimate, p.value)
...
```{r}
plt<- plot(roc.glmnet(cv.mle$pred, newy = y,family='binomial'),type="s")+title(main = "MLE ROC") + text(0.8,0.1,paste("AUC=", round(cv.mle.perf,5)))
```
...
# Coefficients
lasso_estimate <- as.vector(coef(cv_lasso, s=cv_lasso$lambda.min)) 
...
adapt_coef <- as.vector(coef(adapt_lasso,s='lambda.min')) 
adapt_coef <- as.data.frame(adapt_coef)
row.names(adapt_coef) = row.names(ci_lasso_df)
...
```{r}
plot(NA, ylim=1.25*range(ci.lassopost[,1:3]), xlim=c(0,nrow(ci.lassopost)), ylab='95% CI', xlab='', main='')
...
ci_lasso_df <- as.data.frame(ci.lassopost) 

plot_CIs <- ggplot(ci_lasso_df, aes(x = 1:133, y = exp(estimate))) +
            geom_point(size = 0.8, pch = 21, fill= "black", alpha = 0.8) +
            geom_errorbar(aes(ymin = exp(ci.low) , ymax = exp(ci.up)), lwd = 0.2, width = 0) +
            xlab("Variable index") +
            ylab("Odds ratio") +
            ggtitle("LASSO Point Estimates and 95% Confidence Intervals")

```

```{r}
### Obtain CIs & P-values for Adpative LASSO

# Helper function
adapt_lassopost <- function(y,x, method.lambda, verbose = FALSE) {
#Run LASSO + post-selection inference on selected coefficients. lambda set via 10-fold cross-validation
#Input
# - y: response variable
# - x: design matrix
...
if (method.lambda == 'cv') {
cvfit= cv.glmnet(x=xstd, y=ystd, nfolds=10,exact=TRUE)
lasso_coef <- 1/abs(matrix(coef(cvfit, s=cvfit$lambda.min)))
lasso_coef[lasso_coef== Inf] <- 999999999
cvfit <- cv.glmnet(x=xstd, y=ystd, alpha = 1, nfolds=10,family = 'binomial',type_measure='auc', penalty.factor=lasso_coef[2:length(lasso_coef)])
lambda= cvfit$lambda.min
} else if (method.lambda == 'bic') {
lambda= lasso.bic(y=ystd, x=xstd)$lambda.opt
} else { stop("method.lambda not implemented") }
...
adapt.lassopost= lassopost(y, X, method.lambda='cv')
...
accuracy_results <- data.frame(Model = c("Standard Logisitic", "LASSO", "Adaptive LASSO"), 
           ...
           "Optimal lambda" = c("-", round(cv_lasso$lambda.min,5), round(adapt_lasso$lambda.min,5)))

## Make table for Latex
print(xtable(accuracy_results, type = "latex"))

### Coefficients and confidence intervals
results_inference <- results_table %>%
  mutate(LASSO_estimate = round(c(NA, ci.lassopost[,1]),4),
         LASSO_estimate = ifelse(LASSO_estimate == 0, 0.0000, round(exp(LASSO_estimate),4)),
         LASS0_CI_lower = round(c(NA, ci.lassopost[,2]),4),
         LASS0_CI_lower = ifelse(LASS0_CI_lower == 0, 0, round(exp(LASS0_CI_lower),4)),
         LASS0_CI_upper = round(c(NA, ci.lassopost[,3]),4),
         LASS0_CI_upper = ifelse(LASS0_CI_upper == 0, 0, round(exp(LASS0_CI_upper),4)),
         LASSO_pvalue = round(c(NA,ci_lassopost[,4]),4))

## Make table for Latex
print(xtable(results_inference, type = "latex", digits = 3), include.rownames = FALSE)  
