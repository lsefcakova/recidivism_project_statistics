  ...
  if (ncol(x)>0) {  #if there are some covariates
    for (k in 1:K) {
        ...
        if (criterion=='cv') {
            ...
            pred[sel] <- fit$coef[1] + x[sel,,drop=FALSE] %*% matrix(fit$coef[-1],ncol=1) 
        } else if (criterion=='BMA'){
            
            fit <- modelSelection(y=as.matrix(y[!sel]),x=x[!sel,,drop=FALSE], priorCoef=zellnerprior(taustd=1), priorDelta=modelbbprior(1,1))
            ci.bayesreg1 <- coef(fit.bayesreg)[,1]
            pred[sel] <- predict(fit,newx = x[sel,,drop=FALSE])
            #pred[sel] <-ci.bayesreg1[1] + as.matrix(x[sel,,drop=FALSE]) %*% matrix(ci.bayesreg1[-1],ncol=1)
